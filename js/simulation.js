export function createSimulation(root, rocketStats = { parts: [], fuel: 100 }) {
    ...
    function createSmoke() {
        ...
        requestAnimationFrame(() => {
            smoke.style.transform = \`translateX(\${(Math.random() - 0.5) * 80}px) translateY(-\${100 + Math.random() * 120}px) scale(2.8)\`;
    ...
    function simulationLoop() {
        ...
        if (fuel > 0) {
            ...
            // Keep rocket visible
            rocket.style.transform = 'translateX(-50%) translateY(-' + (rocketOffset - cameraOffset) + 'px)';
